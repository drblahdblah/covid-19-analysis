...
class CoronaAnalysis:
    ...
    def load_raw_data(self) -> pd.DataFrame:
        ...
        if self.data_type == 'world' and self.case_type == 'case':
            raw_data = pd.read_csv(
                f'~/PyCharmProjects/covid-19-analysis/data/COVID-19/csse_covid_19_data/'
            ...
            raw_data = pd.read_csv(
                '~/PyCharmProjects/covid-19-analysis/data/COVID-19/csse_covid_19_data/'
        ...
        elif self.data_type == 'aus' and self.case_type == 'death':
            raw_data = pd.read_csv(
                f'~/PyCharmProjects/covid-19-analysis/data/COVID-19/csse_covid_19_data/'
                f'csse_covid_19_time_series/time_series_covid19_deaths_global.csv')
            raw_data = raw_data.loc[raw_data['Country/Region'] == 'Australia']
        ...
        if self.data_type == 'world':
            ...
            countries_to_drop = ['Holy See', 'Congo (Brazzaville)', 'Congo (Kinshasa)',
                                 'Diamond Princess',
                                 'MS Zaandam', 'Kosovo', 'Timor-Leste', 'West Bank and Gaza',
                                 'Western Sahara']
            raw_data_filtered = raw_data_filtered.loc[
                ~raw_data_filtered['Country/Region'].isin(countries_to_drop)]

        elif self.data_type == 'usa' and self.case_type == 'case':
            useful_cols = [col for col in raw_data.columns if
                           col not in ['UID', 'iso2', 'iso3', 'code3',
            ...
            useful_cols = [col for col in raw_data.columns if
                           col not in ['Lat', 'Long', 'Country/Region']]
            raw_data_filtered = raw_data[useful_cols]
    ...
    def run_corona_analysis(self):
        """
        Main function to run analyses of World, USA, and Australian corona virus data from
        the JHU database
        """
        ...
        pl_acceleration = cts.calculate_power_law_acceleration(df_to_transform=pl_slope,
                                                               period=period,
                                                               groupby_list=groupby_list,
                                                               rise=f'slope of log10'
                                                                    f'(New cases per week)',
                                                               run='log10(Total cases)')
        pl_acceleration = \
            (pl_acceleration
             .rename(columns={"slope of slope of log10(New cases per week)":
    ...
    def stack_data_for_dashboard(self, df_to_stack):
        """
        Method to arrange data in a dataframe in a way that allows for representation in the
        dashboard.
        :param df_to_stack:
        :return:
        """
    ...
    def assign_continent_to_df(self, df_add_continent: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.data_type == 'world':
            df_add_continent.loc[
                df_add_continent['Country/Region'] == 'US', 'Country/Region'] = 'USA'
            df_add_continent.loc[
                df_add_continent['Country/Region'] == 'Burma', 'Country/Region'] = 'Myanmar'
            df_add_continent.loc[
                df_add_continent[
                    'Country/Region'] == 'Cote d\'Ivoire', 'Country/Region'] = 'Ivory Coast'
            df_add_continent.loc[df_add_continent[
                                     'Country/Region'] == 'Korea, South', 'Country/Region'] = \
                'South Korea'
            df_add_continent.loc[
                df_add_continent['Country/Region'] == 'Taiwan*', 'Country/Region'] = 'Taiwan'

            df_add_continent['Continent'] = (
                df_add_continent.apply(
                    lambda x: self.apply_continent_to_country(x['Country/Region']),
                    axis=1
                )
            )
    ...
    def write_to_csv(self, df_to_write: pd.DataFrame, path_to_write_to: str, file_name: str):
        ...
        if self.data_type == 'world':
            if self.case_type == 'case':
                write_path_cases = f"../data/output/world/{self.processing_date}/" \
                                   f"cases/{path_to_write_to}"
            ...
            if self.case_type == 'death':
                write_path_deaths = f"../data/output/world/{self.processing_date}/" \
                                    f"deaths/{path_to_write_to}"
            ...
            write_path_cases = f"../data/output/aus/{self.processing_date}/" \
                               f"deaths/{path_to_write_to}"
            if os.path.exists(write_path_cases):
                print(f'Writing data out to path: {write_path_cases}')
                df_to_write.to_csv(write_path_cases + f'/{file_name}')
            ...
            if self.case_type == 'case':
                write_path_cases = f"../data/output/usa/{self.processing_date}/" \
                                   f"cases/{path_to_write_to}"
                if os.path.exists(write_path_cases):
                    print(f'Writing data out to path: {write_path_cases}')
                    df_to_write.to_csv(write_path_cases + f'/{file_name}')
            ...
            if self.case_type == 'death':
                write_path_deaths = f"../data/output/usa/{self.processing_date}/" \
                                    f"deaths/{path_to_write_to}"
                if os.path.exists(write_path_deaths):
                    print(f'Writing data out to path: {write_path_deaths}')
                    df_to_write.to_csv(write_path_deaths + f'/{file_name}')
